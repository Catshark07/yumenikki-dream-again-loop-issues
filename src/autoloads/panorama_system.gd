...
func _setup() -> void:
	...
	panorama_update.do_on_notify(["SCENE_CHANGE_SUCCESS"], func(): 
		eqn = Vector2.ZERO
		RenderingServer.global_shader_parameter_set("uv_offset",  0))
...
func _physics_update(_delta: float) -> void:
	...
	if !warp_check and CameraHolder.instance != null:
		eqn += (CameraHolder.instance.vel) / Application.get_viewport_dimens()
