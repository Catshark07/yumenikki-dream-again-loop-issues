...
func _ready() -> void:
	...
	self.global_position.x = r * cos((1 / distance * PI) * (-CameraHolder.instance.cam.global_position.x + cam_initial_displacement)) + poi.position.x
	self.global_position.z = r * sin((1 / distance * PI) * (-CameraHolder.instance.cam.global_position.x + cam_initial_displacement)) + poi.position.z
	...
	if poi: # if node3D exists:
		...
		if CameraHolder.instance.cam: 	
			...
			self.global_position.x = lerp(
				...
				r * cos((1 / distance * PI) * (-CameraHolder.instance.cam.global_position.x + cam_initial_displacement)) + poi.position.x,
				delta * speed)
			self.global_position.z = lerp(
				self.global_position.z,
				r * sin((1 / distance * PI) * (-CameraHolder.instance.cam.global_position.x + cam_initial_displacement)) + poi.position.z,
				delta * speed)
