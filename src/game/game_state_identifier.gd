...
func _ready() -> void: 
	...
	state_requests_listener.do_on_notify(
		["SPECIAL_INVERT_START_REQUEST"], func(): play_mode_fsm._change_to_state_or_fsm("special_invert_scene"))
	state_requests_listener.do_on_notify(
		["CUTSCENE_START_REQUEST"], func(): play_mode_fsm._change_to_state_or_fsm("cutscene"))
	
	state_requests_listener.do_on_notify(
		["SPECIAL_INVERT_END_REQUEST", 
		"CUTSCENE_END_REQUEST",
		"SCENE_CHANGE_REQUEST"], func(): play_mode_fsm._change_to_state_or_fsm("play")) #?
