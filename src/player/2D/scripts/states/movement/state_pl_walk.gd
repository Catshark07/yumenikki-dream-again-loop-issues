...
func _state_enter() -> void:
	...
	if sentient.get_behaviour().auto_sprint: 
...
func _state_update(_delta: float) -> void:
	if 	(sentient.desired_speed >= \
		sentient.get_calculated_speed(sentient.sprint_multiplier) and sentient.can_sprint) or \
		sentient.get_behaviour().auto_sprint:
			request_transition_to("sprint")
