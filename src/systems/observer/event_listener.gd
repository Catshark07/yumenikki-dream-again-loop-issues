...
func on_notify(_id: String) -> void: # --- called from the event_manager.
	_id = _id.to_upper()
	if _id in GameManager.EventManager.event_ids:
...
func do_on_notify(_event_id: String, _do := Callable()) -> void: 
	...
	if _event_id in GameManager.EventManager.event_ids and _event_id in events_listening_to:
		actions[_event_id] = Callable(_do)

func listen_to_event(_event_id: String):
	_event_id = _event_id.to_upper()
	GameManager.EventManager.add_listener(self, _event_id)
