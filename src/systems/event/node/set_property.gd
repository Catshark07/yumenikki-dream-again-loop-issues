...
func _validate_property(property: Dictionary) -> void:
	var hidden_properties: PackedStringArray = [
		"property_value",
		"new_value"
		]
	...
	if property["name"] in hidden_properties: 
		if node == null: 			
			property["usage"] = PROPERTY_USAGE_NONE # if the node is null, we hide these props.
		elif !property["name"] in adaptable_properties:				
...
func _execute() -> void:
	...
	print(node.get_indexed(property_name))
	super()
