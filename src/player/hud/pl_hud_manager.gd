...
func events_setup() -> void:
	...
	on_invert.do_on_notify(["SPECIAL_INVERT_CUTSCENE_BEGIN"], func(): if indicators: show_ui(indicators, false))
	on_invert.do_on_notify(["SPECIAL_INVERT_CUTSCENE_END"], func(): if indicators: show_ui(indicators, true))
